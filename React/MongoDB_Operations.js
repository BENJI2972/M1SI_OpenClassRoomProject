 ...
 */
var MongoDB_Operations = (function () {
    function MongoDB_Operations(url, dbName, collectionName) {
        this.url = url;
        this.dbName = dbName;
     ...
     */
    MongoDB_Operations.prototype.connectionMongoDB = function (functionToDo, jsonQuery, miseAJourReact) {
        var that = this;
     ...
     */
    MongoDB_Operations.prototype.findDocuments = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.find(jsonQuery).toArray(function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
        });
    };
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.insertOneDocument = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.insertOne(jsonQuery, function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            assert.equal(1, rows.insertedCount);
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
        });
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.insertManyDocuments = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.insertMany(jsonQuery, function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            assert.equal(jsonQuery.length, rows.insertedCount);
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
        });
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.updateOneDocument = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.updateOne(jsonQuery[0], jsonQuery[1], function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.updateManyDocuments = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.updateMany(jsonQuery[0], jsonQuery[1], function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.deleteOneDocument = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.deleteOne(jsonQuery, function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
     ...
     * @param callback is the callback function, called when this class finished its job
     */
    MongoDB_Operations.prototype.deleteManyDocuments = function (db, jsonQuery, callback, miseAJourReact) {
        var collection = db.collection('oeuvres');
        collection.deleteMany(jsonQuery, function (err, rows) {
            assert.equal(err, null); // raise an exception if there is an error
            console.log(rows); // found the following mongoDB records
            callback(rows);
            miseAJourReact(rows);
    ...
    MongoDB_Operations.prototype.setDbName = function (dbName) {
        this.dbName = dbName;
    };
    MongoDB_Operations.prototype.setCollectionName = function (collectionName) {
        this.collectionName = collectionName;
    };
    return MongoDB_Operations;
}());
exports.MongoDB_Operations = MongoDB_Operations;
//=============== Test part begin =====================
